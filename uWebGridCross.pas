{-----------------------------------------------------------------------------
 ...
 Purpose:   本单元提供类似frxCrossView的功能，也采用类似的使用方法，产生一个
            ...
            使用时，用户创建TWebCrossView对象，设置列数和标题行数，之后填入数据，
            调用CrossGrid或CrossPage取回格式化的HTML表格或HTML页面。再后，用浏览
            器显示出来即可。
            例如：
            ------------------------------------
                ...
                with wcv do
                begin
                    AddRow(['日期', '监测仪器', '监测仪器', '观测数据', '观测数据',
                            '备注']);
                    AddRow(['日期', '仪器编号', '子传感器', '开合度', '温度', '备注']);
                    AddRow([today, 'X', 'X-1', 25, 24, '']);
                    AddCaptionRow(['数据2', '数据2', '数据2', '内容']);
...
const
    ...
    { 表格代码 }
    htmTableCode = '<table BORDER=0 CELLSPACING=0 CELLPADDING=0 BGCOLOR=#ADD8E6 class="DataGrid">'#13#10 //BGCOLOR=#ECE9D8
        + '@Caption@'
        + '@Rows@'
        + '</table>';
...
begin
    ...
    begin
        case VarType(FValue) of                                                 //
            ...
            varSingle, varDouble, varCurrency: Result := FormatFloat('0.00',
                    FValue);
            varDate: Result := FormatDateTime('yyyy-mm-dd hh:mm:ss', FValue);
